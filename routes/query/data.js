...
exports.getOutputGraphDataByBank = function(user_id, callbackFunc){
  ...
  console.log("SQL:getInputGraphDataByBank  "+sql);
...
exports.getInOutGraphDataByItemType = function(user_id, in_out_type, callbackFunc){
  if(!user_id) return null;
  var sql = "SELECT ITEM_SETTING.NAME AS NAME, ABS(COALESCE(SUM(PRICE),0)) AS VALUE FROM ITEM_SETTING LEFT JOIN PAYMENT ON (ITEM_SETTING.ITEM_SETTING_ID = PAYMENT.ITEM_ID AND ITEM_SETTING.PERSON_ID = PAYMENT.PERSON_ID) WHERE ITEM_SETTING.IN_OUT_TYPE = "+in_out_type+" AND PAYMENT.PERSON_ID = "+user_id+" AND PAYMENT.DEL_FLG = 0 AND ITEM_SETTING.DEL_FLG = 0 GROUP AND ITEM_SETtING.ITEM_SETTING_ID > 0 BY ITEM_SETTING.NAME;";
  console.log("SQL:getInOutGraphDataByItemType  "+sql);
  conn.getConnection().query(sql, function(err, result) {
    if (err){
      console.log("SQL ERROR:"+err);
      return null;
