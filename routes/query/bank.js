...
exports.getBankList = function(user_id, callbackFunc){
  ...
  logger.loggerRequestFunc("SQL:getBankList  "+sql);
...
exports.insertBank = function(item, callbackFunc){
  ...
  logger.loggerRequestFunc("SQL:insertBank  "+_sql);
  conn.getConnection().query(_sql, function(err, _result){
    ...
    logger.loggerRequestFunc("SQL:insertBank  "+sql);
    conn.getConnection().query(sql, function(err, result) {
      if (err){
        logger.loggerRequestFunc("SQL ERROR:"+err);
        return null;
...
exports.updateBank = function(item, callbackFunc){
  ...
  logger.loggerRequestFunc("SQL:updateBank  "+sql);
  conn.getConnection().query(sql, function(err, result) {
    if (err){
      logger.loggerRequestFunc("SQL ERROR:"+err);
      return null;
...
exports.deleteBank = function(item, callbackFunc){
  ...
  logger.loggerRequestFunc("SQL:deleteBank  "+sql);
  conn.getConnection().query(sql, function(err, result) {
    if (err){
      logger.loggerRequestFunc("SQL ERROR:"+err);
      return null;
